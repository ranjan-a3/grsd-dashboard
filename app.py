    ...
    with h_col2:
        ...
        if heat_rows:
            ...
            styled = (rank_df.style
                      .applymap(style_m, subset=["Momentum"])
...
with tab_ta:
    ...
    if ticker_input:
        ...
        if ta_df.empty:
            ...
            rs_val  = rs_score(ta_df["Close"], ta_bench["Close"])
            ind["rs_value"] = rs_val
  ...
  <div style="font-size:0.74rem;color:{T['muted']};margin-top:4px;">
    ...
    for etf_key in US_SECTORS:
        if etf_key in us_sect_prices and not us_sect_prices[etf_key].empty:
            r = rs_score(us_sect_prices[etf_key]["Close"], us_bench["Close"])
    ...
    if us_rs_rows:
        ...
        styled_us = (us_rs_df.style
                     .applymap(clr_rs,  subset=["RS% (3M)"])
                     .applymap(clr_stg, subset=["Stage"])
                     .format({"RS% (3M)": "{:+.1f}%"}))
    ...
    for sec_key, info in INDIA_SECTORS.items():
        if sec_key in india_sect_prices and not india_sect_prices[sec_key].empty:
            r = rs_score(india_sect_prices[sec_key]["Close"], india_bench["Close"])
